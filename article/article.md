...
This is just one cross-platform compiler convenient enough
...
What if we one project depends on another one? It's not the case for our simple example, because FPC will automatically build the units and libraries required to build dependent units. But it can be the case in more complex situations. No MSBuild mechanisms would work in the general case, so we need to create such a mechanism from scratch. Fortunately, this is quite a simple thing.

Let's pretend we have a project depending on another project, so the project our project depends on should be built first.

Let's introduce a new item set and name it <code>References</code>:

```{lang=XML}
&lt;Project Sdk="Microsoft.NET.Sdk"&gt;

    &lt;ItemGroup&gt;
        &lt;UnitPath Include="$(ProjectDir)../project.Shared" /&gt;
    &lt;/ItemGroup&gt;

    &lt;ItemGroup&gt;
        &lt;InputFiles Include="$(InputMask)" /&gt;
        &lt;References Include="../project.Shared/a.project" /&gt;
    &lt;/ItemGroup&gt;

&lt;/Project&gt;
```

Now, we need to create a simple recursion. Each project should push the build of the referenced project before building itself. As the tasks withing a target are executed sequentially, it is easy to organize:
...
Here is the important point: we need to pass the `$(MasterDir)` value transparently. This value is generated by the target `"Build"`, executed only once by the master project. []() SA???

### Circular Dependencies

What if we make a mistake and create a circular dependencies between projects? No problem. MSBuild will detect this problem at the moment of the recursive call to the `MSBuild` task and provide a clear error message.

This is an intrinsic MSBuild feature not related to a particular targets or properties schema. It is also related to optimization: MSBuild won't allow re-building of the same project if more than one project depend on it.
